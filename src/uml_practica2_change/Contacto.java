...
public class Contacto {
	...
	static Direccion crearDireccion(Scanner sc) {
		System.out.println("\n--- Dirección ---");

		TipoVia tipoVia = Direccion.elegirTipoVia(sc);

		int numero = Main.leerEntero(sc, "Número: ");

		String bloque = Main.leerTexto(sc, "Bloque (opcional): ");
		String escalera = Main.leerTexto(sc, "Escalera (opcional): ");
		String portal = Main.leerTexto(sc, "Portal (opcional): ");
		String letra = Main.leerTexto(sc, "Letra (opcional): ");

		Direccion d = new Direccion(tipoVia, numero, bloque, escalera, portal, letra);
		return d;
	}

	static Telefono crearTelefono(Scanner sc) {
		System.out.println("\n--- Teléfono ---");

		String numero = Main.leerTextoNoVacio(sc, "Número de teléfono: ");
		TipoTelefono tipo = Telefono.elegirTipoTelefono(sc);
